...
// Interface que define as informações que ficam disponiveis de forma global na aplicão
...
const tasksData = '@MyTasks:Tasks'; // Chave para o async storage
...
export const TasksProvider: React.FunctionComponent<IProps> = ({children}) => {
  ...
  const addTask = async (task: ITask) => {
    try {
      const newTaskList = [...data, task]; // Error aquiiiiii
...
export function useTaskList(): ITasksContext {
  ...
  // Só pode ser chamado dentro do Provider
  if (!context) {
